      ...
      <div className="studio-container">
        ...
        <div className={`workspace-studio ${activeTab !== 'oven' ? 'mobile-hidden' : ''}`}>
           ...
           <div className="history-studio">
                ...
                {/* v1.2.53: STUDIO HEALTH SIGNAL - Session Bridge */}
                <div 
                   onClick={async () => {
                      try {
                         const diag = await api.get('/diag/vault-status');
                         alert(`EMPIRE VAULT 1.2.53 (SESSION):\nSignal: ${diag.recommendation}\nVault Init: ${diag.initStatus}\nURL Mirror: ${diag.urlMirror}\n\nIDENTITY: ${diag.identity?.user}@${diag.identity?.db}`);
                ...
                >
                   ...
                   STUDIO SYNC [v1.2.53]: {history.length} RECORDS IN VAULT (CLICK TO RESTORE)
                </div>
